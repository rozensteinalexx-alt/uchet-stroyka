...
def process_invoice(uploaded_file):
    ...
    with st.status(f"🧠 ИИ читает чек ({CURRENT_MODEL_NAME})...", expanded=True) as status:
        ...
        status.write("✅ Разбираем данные (без доставки)...")
        model = genai.GenerativeModel(CURRENT_MODEL_NAME)
        
        # ОБНОВЛЕННЫЙ ЗАПРОС: ИГНОРИРОВАТЬ ДОСТАВКУ
        prompt = f"""
        Роль: Сметчик. Задача: Извлечь данные из чека.
        
        Правила:
        1. Единицы измерения (unit) строго на русском: "шт", "уп", "м", "кг", "компл".
        2. Категории только из списка: {CATEGORIES}
        3. ВАЖНО: ИГНОРИРУЙ услуги (Доставка, Подъем, Разгрузка, Вывоз мусора). Выписывай только МАТЕРИАЛЫ.
